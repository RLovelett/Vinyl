...
public struct RequestMatcherRegistry {
    ...
    private static func matcherForType(_ requestMatcherType: RequestMatcherType) -> RequestMatcher {
...
private struct QueryRequestMatcher: RequestMatcher {
    func match(_ aRequest: Request, anotherRequest: Request) -> Bool {
        ...
        let aRequestItems = queryItems(aRequest).sorted(isOrderedBefore: >)
        let anotherRequestItems = queryItems(anotherRequest).sorted(isOrderedBefore: >)
