...
final class PlaybackSequenceMatchTypeMethodTests: XCTestCase {
    ...
    func testSameMethod() {
        XCTAssertTrue(Turntable.PlaybackSequence.MatchType.method.match(track, with: self.request))
    }

    func testDifferentMethod() {
        self.request.httpMethod = "GET"
        XCTAssertFalse(Turntable.PlaybackSequence.MatchType.method.match(track, with: self.request))
    }
...
final class PlaybackSequenceMatchTypeURLTests: XCTestCase {
    ...
    func testExactMatch() {
        XCTAssertTrue(Turntable.PlaybackSequence.MatchType.url.match(track, with: self.request))
    }

    func testPurposelyMismatched() {
        self.request.url = URLComponents.arbitrary.generate.url
        XCTAssertFalse(Turntable.PlaybackSequence.MatchType.url.match(track, with: self.request))
    }
...
final class PlaybackSequenceMatchTypePathTests: XCTestCase {
    ...
    func testMatchingPath() {
        XCTAssertTrue(Turntable.PlaybackSequence.MatchType.path.match(track, with: self.request))
    }
    ...
    func testDifferentPath() {
        ...
        self.request.url = self.components.url
        XCTAssertFalse(Turntable.PlaybackSequence.MatchType.path.match(track, with: self.request))
    }
...
final class PlaybackSequenceMatchTypeQueryTests: XCTestCase {
    ...
    func testMatchingQuery() {
        XCTAssertTrue(Turntable.PlaybackSequence.MatchType.query.match(track, with: self.request))
    }
    ...
    func testRequestWithoutQueries() {
        ...
        self.request.url = self.components.url
        XCTAssertFalse(Turntable.PlaybackSequence.MatchType.query.match(track, with: self.request))
    }
...
final class PlaybackSequenceMatchTypeHeaderTests: XCTestCase {
    ...
    let matcher = Turntable.PlaybackSequence.MatchType.headers
...
final class PlaybackSequenceMatchTypeBodyTests: XCTestCase {
    ...
    func testMatchingBody() {
        XCTAssertTrue(Turntable.PlaybackSequence.MatchType.body.match(track, with: self.request))
    }

    func testRequestWithoutBody() {
        let trackRequest = Track.Request(from: url, using: .head, with: reqHeader, sending: nil)
        let track = Track(request: trackRequest, response: trackResponse)
        XCTAssertFalse(Turntable.PlaybackSequence.MatchType.body.match(track, with: self.request))
    }
...
final class PlaybackSequenceMatchTypeCustomTests: XCTestCase {
    ...
    func testMatchingCustomFunction() {
        let matcher = Turntable.PlaybackSequence.MatchType.custom(using: customMatching)
